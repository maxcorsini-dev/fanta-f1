...
async function fetchCalendar(year=2026) {
  ...
  for (const race of races) {
    ...
    if (!ex) await run(`INSERT INTO races (season_id,round,name,circuit,country,date,deadline,jolpica_round) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
      [season.id, parseInt(race.round), race.raceName, race.Circuit.circuitName, race.Circuit.Location.country, race.date, getDeadline(race.date), parseInt(race.round)]);
  }
  return races.length;
}
...
async function fetchDrivers(year=2026) {
  ...
  for (const d of drivers) {
    ...
    if (!ex) await run('INSERT INTO drivers (season_id,number,code,full_name,team,jolpica_id) VALUES ($1,$2,$3,$4,$5,$6)',
...
async function fetchRaceResults(year, round) {
  ...
  for (const r of raceData.Results) {
    ...
    await run('INSERT INTO race_results (race_id,driver_id,position,is_dnf,is_pole,caused_incident) VALUES ($1,$2,$3,$4,$5,0)',
      [race.id, driver.id, isDnf?null:parseInt(r.position), isDnf?1:0, r.Driver.driverId===poleId?1:0]);
  }
  ...
  for (const race of calRes.data.MRData.RaceTable.Races) {
    ...
    try {
      ...
      for (const r of rr.data.MRData.RaceTable.Races[0]?.Results||[]) {
        const isDnf = r.status!=='Finished' && !r.status.startsWith('+');
        await run('INSERT INTO previous_year_results (race_name,driver_code,position,is_dnf) VALUES ($1,$2,$3,$4)',
          [race.raceName, r.Driver.code, isDnf?null:parseInt(r.position), isDnf?1:0]);
      }
